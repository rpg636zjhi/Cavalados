...
class Enchantment
{
    ...
    public static function getEnchantAbility(Item $item)
    {
        ...
        if ($item->isArmor()) {
            if ($item instanceof ChainBoots || $item instanceof ChainChestplate || $item instanceof ChainHelmet || $item instanceof ChainLeggings) {
                return 12;
            }
            if ($item instanceof IronBoots || $item instanceof IronChestplate || $item instanceof IronHelmet || $item instanceof IronLeggings) {
                return 9;
            }
            if ($item instanceof DiamondBoots || $item instanceof DiamondChestplate || $item instanceof DiamondHelmet || $item instanceof DiamondLeggings) {
                return 10;
            }
            if ($item instanceof LeatherBoots || $item instanceof LeatherTunic || $item instanceof LeatherCap || $item instanceof LeatherPants) {
                return 15;
            }
            if ($item instanceof GoldBoots || $item instanceof GoldChestplate || $item instanceof GoldHelmet || $item instanceof GoldLeggings) {
                return 25;
            }
        }

        if ($item->isTool()) {
            if ($item instanceof WoodenAxe || $item instanceof WoodenHoe || $item instanceof WoodenPickaxe || $item instanceof WoodenShovel || $item instanceof WoodenSword) {
                return 15;
            }
            if ($item instanceof StoneAxe || $item instanceof StoneHoe || $item instanceof StonePickaxe || $item instanceof StoneShovel || $item instanceof StoneSword) {
                return 5;
            }
            if ($item instanceof DiamondAxe || $item instanceof DiamondHoe || $item instanceof DiamondPickaxe || $item instanceof DiamondShovel || $item instanceof DiamondSword) {
                return 10;
            }
            if ($item instanceof IronAxe || $item instanceof IronHoe || $item instanceof IronPickaxe || $item instanceof IronShovel || $item instanceof IronSword) {
                return 14;
            }
            if ($item instanceof GoldAxe || $item instanceof GoldHoe || $item instanceof GoldPickaxe || $item instanceof GoldShovel || $item instanceof GoldSword) {
                return 22;
            }
        }
