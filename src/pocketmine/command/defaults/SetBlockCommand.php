...
class SetBlockCommand extends VanillaCommand
{
    ...
    public function execute(CommandSender $sender, $currentAlias, array $args)
    {
        ...
        if ($itemblock instanceof ItemBlock) {
            ...
            if ($x[0] === "~") {
                if ((is_numeric(trim($x, "~")) || trim($x, "~") === "") && ($sender instanceof Player)) {
                    $x = (int)round(trim($x, "~") + $sender->x);
                }
            } elseif (is_numeric($x)) {
                $x = (int)round($x);
            } else {
            ...
            if ($y[0] === "~") {
                if ((is_numeric(trim($y, "~")) || trim($y, "~") === "") && ($sender instanceof Player)) {
                    $y = (int)round(trim($y, "~") + $sender->y);
                }
                if ($y < 0 || $y > 128) {
                    return false;
                }
            ...
            if ($z[0] === "~") {
                if ((is_numeric(trim($z, "~")) || trim($z, "~") === "") && ($sender instanceof Player)) {
                    $z = (int)round(trim($z, "~") + $sender->z);
                }
            } elseif (is_numeric($z)) {
                $z = (int)round($z);
            } else {
