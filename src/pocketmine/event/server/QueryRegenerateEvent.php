...
class QueryRegenerateEvent extends ServerEvent
{
    ...
    public function __construct(Server $server, $timeout = 5)
    {
        ...
        if ($server->isDServerEnabled() && $server->dserverConfig["queryMaxPlayers"]) {
            $pc = $server->dserverConfig["queryMaxPlayers"];
        } elseif ($server->isDServerEnabled() && $server->dserverConfig["queryAllPlayers"]) {
            $pc = $server->getDServerMaxPlayers();
        } else {
            $pc = $server->getMaxPlayers();
        }

        if ($server->isDServerEnabled() && $server->dserverConfig["queryPlayers"]) {
